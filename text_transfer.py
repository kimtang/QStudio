...
def q_sender(repl, text, view=None, repl_view=None):
    text0 = ":executeK4Query(\"" + raw(text)+ "\");\n";
    if text[0:len(':addSetServer')] == ':addSetServer' :
        text0 = text
    if text[0:len(':exportAsExcel')] == ':exportAsExcel' :
        text0 = text
    if text[0:len(':exportAsTxt')] == ':exportAsTxt' :
        text0 = text
    if text[0:len(':exportAsCSV')] == ':exportAsCSV' :
        text0 = text
    if text[0:len(':chart')] == ':chart' :
        text0 = text
